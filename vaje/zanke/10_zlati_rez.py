...
def natancni_priblizek(natancnost):
    phi = 1
    naslednji = naslednji_priblizek(phi)
    while abs(naslednji - phi) >= natancnost:
        phi = naslednji
        naslednji = naslednji_priblizek(phi)
    return naslednji
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoxMDAwLCJ1c2VyIjoxMTQ0Nn0:1w6wQm:eRW5CPOU0P9j28bObBVovybjZzEyOA476w56Oubhcd4"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoxMDAxLCJ1c2VyIjoxMTQ0Nn0:1w6wQm:AKJhrYy63irEtQqaCfc1D0tKy4XWrRe8DuaAZzx_OX8"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoxMDAyLCJ1c2VyIjoxMTQ0Nn0:1w6wQm:85P3IGx28IpaRGV-6CEn2lzJ3n6MqeICVAnBwIrHn_A"
        try:
